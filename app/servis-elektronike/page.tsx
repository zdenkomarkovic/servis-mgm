  ...
  return (
    <div className=" py-32 mx-auto space-y-6 md:space-y-12">
      <div className="container px-2 md:px-4 mx-auto space-y-6 md:space-y-12">
        <h1>{pages.electronics.title}</h1>
        <p className="px-32 text-center">{pages.electronics.text}</p>
        <div className="flex gap-3 md:gap-12 items-center justify-center">
          <div>
            {pages.electronics.description && (
              <p className="font-bold text-primary">
                {pages.electronics.description}
              </p>
            )}
            <ul>
              {pages.electronics.lines.map((line, i) => {
        ...
        <p className="px-32 text-center text-primary">
          {pages.electronics.workflow}
        </p>
      </div>
      <CTA text={pages.electronics.cta} />
      <Cards2Kosina
        title={"Zašto izabrati nas"}
        data={pages.electronics.whyus}
      />
    </div>
  );
};
